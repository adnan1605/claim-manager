...
public class PolicyController {
	...
	public Policy getEligibleBenefits(@PathVariable int memberId,@PathVariable int policyId
			 ...
			 * service.getEligibleBenefits(policyId, memberId); }else { throw new
			 * Exception("no Benefits"); }
			 */
		return service.getEligibleBenefits(policyId, memberId);
	}
	@GetMapping("/getEligibleClaimAmount/{policyId}/{memberId}/{benefits}")
	public int getEligibleClaimAmount(@PathVariable int benefits,@PathVariable int policyId,@PathVariable int memberId) throws Exception{
		/*
		 * if(client.authorizeTheRequest(requestTokenHeader)) { return
		 * service.getEligibleClaimAmount(policyId, memberId, benefits); }else { throw
		 * new Exception("Can't claim"); }
		 */
